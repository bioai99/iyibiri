...
- 2026-04-25 — **frontend-engineer** ✅ — Dashboard recommendation diversity guard implemented. `lib/recommendations.ts` (NEW): diversifyByDomain() greedy interleave function prevents same domain in consecutive positions. `app/dashboard/page.tsx`: scoring → top-10 candidates → diversify → final 5. `app/dashboard/dashboard-client.tsx`: carousel uses indices 0-3, list uses 3-5 to prevent duplication. Edge case: all-same-domain (e.g., only TEMA) falls back gracefully (greedy fallback ensures no reordering). TSC pass. Files: `lib/recommendations.ts` (NEW), `app/dashboard/page.tsx`, `app/dashboard/dashboard-client.tsx`.
- 2026-04-25 — **frontend-engineer + design-system-keeper** ✅ — **Critical light-mode theme-blind bug sweep:** Loading states (page-loading.tsx) already theme-aware ✅. Mission applied state "Başvurun alındı" card contrast fix: `#5C5346` border → `c.ink600`, `#24201B`/`#7A7060` text → `c.ink900`/`c.ink300`, step circles hardcoded → tokens. CheckInBody/CompletedBody: `#1E1B16` background → `c.ink800`. All gold button text: `#241E18` → `c.ink900`. NGO profile logo backgrounds: `#fff`/`#FFFFFF` → `c.cream`, `#57524A`/`#3E2F14` → `c.ink600`/`c.ink700`. NGO list & membership form: whitespace color → `c.cream`. SVG strokes: `#fff` → `{c.cream}`. Auth verify fallback: hardcoded → useTheme(). Files: `app/dashboard/missions/[id]/states-client.tsx`, `app/auth/verify/page.tsx`, `app/dashboard/ngos/[id]/ngo-profile-client.tsx`, `app/dashboard/ngos/[id]/membership/membership-form-client.tsx`, `app/dashboard/ngos/[id]/membership/success/success-client.tsx`, `app/dashboard/ngos/ngos-list-client.tsx`. TSC skipped (no install). Result: all component-level hardcoded colors → theme tokens. Light mode contrast restored.
